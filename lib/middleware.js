...
exports.getDistTags = async function (name) {
  let output = '';
...
exports.require = async function (name, cwd) {
  ...
  if (nameInfo.isPath) {
    ...
    if (packagePath) {
      if (nameInfo.version) {
        const distTags = await this.getDistTags(nameInfo.fullName);
        ...
        try {
          const packageJson = JSON.parse(fs.readFileSync(pkgJsonPath, { encoding: 'utf8' }));
          debug('targetVersion', targetVersion);
          debug('installedVersion', packageJson.version);
          if (semver.satisfies(packageJson.version, targetVersion)) {
            installed = true;
          }
        } catch (e) {
          debug(e);
