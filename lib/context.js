...
class Context extends EventEmitter {
  ...
  constructor(cli, opts) {
    ...
    this.project = this._getProjectInfo();
    this.conf = configs;
  }
  ...
  async _loadLocalAllPipeline() {
    let localConfigs = await this.loadLocalConfigs();
  ...
  async _loadLocalePipeline(command) {
    ...
    let pipelines = await this._loadLocalAllPipeline();
  ...
  async _mergeRemotePipeline(command, list) {
    ...
    let afterList = pipe.after[command] || [];
  ...
  async loadPipeline(command) {
    command = command || this.command;
    if (!(await this.configIsExists())) return [];
    let list = await this._loadLocalePipeline(command);
