...
function makeConf(options) {
  ...
  let commonOpts = config.options.common || {};
  //可以指定不打包common trunk
  if (!commonOpts.disabled) {
    config.plugins.push(new CommonsChunkPlugin({
      name: commonOpts.name || 'common',
      chunks: chunks,
  ...
  //可以指定entry
  if (options.entry) {
    let _entry = options.entry;
    let _entry_name = _entry.name;
    let _entry_location = _entry.location;
    if (_entry_name && _entry_location) {
      let obj = {};
      obj[_entry_name] = _entry_location;
      config.entry = obj;
    }
